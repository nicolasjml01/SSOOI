...
nawk -v nombre="$nombre" '
	...
	nawk -v fecha="$fecha" '
        BEGIN { n = 5 }  
        $0 ~ "DIA: " fecha {
        # Imprime lineas anteriores a la linea buscada
...
if [[ "$flag_a" == "true" && "$flag_f" == "true" ]]; then
	...
	hora_fc=$(nawk -v especialidad="$especialidad" '
    BEGIN { n = 4 }
    ...
    { buf[NR % n] = $0 }' "$citas" | nawk -v fecha="$fecha" '
        ...
        { buf[NR % n] = $0 }'| grep "HORA_FINAL:" | cut -d ':' -f 2 | tr -d ' ')

	hora_ic=$(nawk -v especialidad="$especialidad" '
    BEGIN { n = 4 }
    $0 ~ "ESPECIALIDAD: " especialidad {
        print
        for (i = 0; i <= n; i++) { getline; print }
    ...
    { buf[NR % n] = $0 }' "$citas" | nawk -v fecha="$fecha" '
        ...
        { buf[NR % n] = $0 }'| grep "HORA_INICIAL:" | cut -d ':' -f 2 | tr -d ' ')
...
ultimo_id=$(nawk -v fecha="$fecha" '
        BEGIN { n = 5 }  # Cambiamos n a 4 para almacenar 4 líneas anteriores
        $0 ~ "DIA: " fecha {
        for (i = NR - n; i < NR; i++) if (i >= 0) print buf[i % n]
        print
        getline; print
        }
        { buf[NR % n] = $0 }' "$citas" | grep "ID: " | cut -d ':' -f 2 | tr -d ' ' | cut -d '_' -f 2 | tail -1)
